...
# Structure:
# 1) Code that cleans orçamento data
#
...
inpdir <- file.path(DROPBOX_PATH, "build", "orçamento", "input")
outdir <- file.path(DROPBOX_PATH, "build", "orçamento", "output")
codedir <- file.path(GITHUB_PATH, "build",  "orçamento", "code")
workfile_dir <- file.path(GITHUB_PATH, "build",  "workfile")
workfile_dir_out <- file.path(DROPBOX_PATH, "build",  "workfile", "output")

##################################
#                                #
#   1) Cleaning orçamento Data 
#                                #
##################################

source(paste0(GITHUB_PATH, "build/orçamento/code/budget.R"))
